...
class Infer:
    ...
    def visualize_samples(
        self, samples, output_path="generated_samples.png", title="Generated Samples"
    ):
...
@click.command()
@click.option(
    "--ckpt_path", default="model.pth", help="Path to the trained model checkpoint"
)
@click.option("--num_samples", default=1000, help="Number of samples to generate")
@click.option(
    "--num_inference_steps",
    default=50,
    help="Number of inference steps for DDIM sampling",
)
@click.option(
    "--seed", default=None, type=int, help="Random seed for reproducible generation"
)
@click.option(
    "--output_path",
...
@click.option(
    "--evaluate", is_flag=True, help="Run evaluation metrics against ground truth"
)
@click.option("--device", default="cuda", help="Device to run inference on")
def main(
    ckpt_path, num_samples, num_inference_steps, seed, output_path, evaluate, device
):
    if seed is not None:
        torch.manual_seed(seed)
        np.random.seed(seed)
    ...
    print(
        f"Generating {num_samples} samples with {num_inference_steps} inference steps..."
    )
